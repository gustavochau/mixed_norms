function [u_iter,stats] = generalADMM(matrices, func_costo, solvers,  params, opts)
...
func_costo_aumentada = @(x1,z1,v1,rho1) func_costo(x1,z1) + (rho1/2)*norm(A(x1)+B(z1)-c+v1,2) - (rho1/2)*norm(v1);
...
%                 norm(v-v_anterior)
    ...
    dual_res = norm(rho*AT(B(z-z_anterior)));
    
    eps_primal= sqrt(length(c))*opts.tol(1)+opts.tol(2)*max([norm(A(x),2) norm(B(z),2) norm(c,2)]); % tolerancia para residuo primal
    eps_dual=sqrt(length(x))*opts.tol(1)+opts.tol(2)*norm(AT(rho*v)); % tolerancia para residuo dual
